...
int bfs( Graph *graph, int *dist, int *pairU, int *pairV )
{
  ...
  //printf("There are %d number of procs.\n", omp_get_num_procs());

  //May be able to use openMP here
  //Pay attention to enqueue: if count or front is not consistent across
  //threads, certain elements may be overwritten.
  int i;
  for( i = 1; i <= graph->l; i++ )
  {
    if( pairU[i] == 0 )
